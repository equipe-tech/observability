...
Signals selecionados sempre exigem avaliação. O fixture `react-web` em `observability/conformance/fixtures/positive/react-web/kit.ts` cria traces e métricas pelo runtime React, envia o lote pela rota NestJS e exige leitura no destino depois do Collector. Quando a correlação de execução usa um atributo de contrato em vez do campo canônico `run.id`, passe `eventRunIdAttribute` e `metricRunIdAttribute` para `telemetryCanaryConformance`. Toda seleção de `browserIngest`, inclusive em `nestjs-api` e `effect-api`, exige `canary.browser-route`.

O fixture `effect-api` em `observability/conformance/fixtures/positive/effect-api/kit.ts` serve uma rota Effect com `httpTelemetry`, emite o evento de contrato dentro de um span filho do span de servidor e registra a métrica pelo handle do runtime. O perfil não passa por evlog; o check `server-events.evlog-collector` fica `not-applicable` e o canário de destino prova a entrega de eventos, spans e métricas pelo Collector. `@equipe-tech/observability-effect/testing` publica `effectDefectBoundaryConformance` para `sentry.unexpected-defects-only` com o dono `effect`.
