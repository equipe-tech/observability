...
O orçamento de visibilidade reserva 200 ms para o `flush_timeout` do Collector, 180.000 ms para `axiomQueryVisibilityMilliseconds` e 11.800 ms para `safetyMarginMilliseconds` (margem de segurança). Os 11.800 ms são uma margem de segurança operacional escolhida, não um valor derivado. A [documentação pública de ingestão do Axiom](https://axiom.co/docs/send-data/) não publica um limite de latência entre ingestão e consulta. Por isso, os 180 segundos são uma tolerância operacional explícita, não uma garantia do provedor. O teste soma os três componentes e exige que o intervalo total de polling seja 192.000 ms.
