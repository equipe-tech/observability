...
- `OBS_NESTJS_ERROR_CATALOG_INVALID` indica que as declarações enumeráveis do catálogo não correspondem à lista `_codes` criada por `defineErrorCatalog`, ou que a declaração nomeada não expõe código, status e mensagem válidos. Reconstrua o catálogo com `defineErrorCatalog` e reinicie o serviço.
