...
Use `evlogAdapter` como o adapter oficial de eventos na composição de `createNodeObservability` ou `createNodeObservabilityFromConfig`. O adapter instala e encerra o logger global. A aplicação não deve chamar `initLogger`, criar outro drain OTLP nem registrar um segundo interceptor de correlação. O `TelemetryInterceptor` cria o único span de servidor e grava `traceId` e `spanId` no logger antes de executar o handler.
