...
CI deriva a data de compatibilidade do relógio UTC e executa o gate depois do build. Metadados `since` de aliases vêm somente do contrato compilado e nunca do histórico Git. O preflight executa o mesmo gate em modo de release antes de empacotar. Para um pacote já publicado, o preflight consulta o packument do npm diretamente, valida a integridade e todos os caminhos do tarball, rejeita links e entradas especiais, extrai a representação canônica de pacote e compara o digest com o baseline. A primeira publicação exige a declaração exata da candidata e não procura um tarball anterior. `release.yml` herda o gate pelo workflow de CI.
